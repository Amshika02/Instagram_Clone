...
function Profile() {
    ...
    useEffect(() => {
        axios.get('http://localhost:3000/profile',profile)
  ...
  return (
    <div className='m-5'>
        {profile ? (
            <div>
                <img src={profile.profile_pic} alt="profile pic" className='profile rounded-circle'/>
        ...
        {followers.length > 0 ? (
            followers.map (follower => {
                <div key={follower.id} className='d-flex my-2'>
                    ...
                    {follower.username}
                <button className='btn btn-secondary ms-auto' onClick={()=>{handleUnFollow }}></button>
                </div>
            })
        ):( 
